...
class CSVPath(click.Path):
    ...
    def convert(self, value, param, ctx):
        values = ()
        repo_dirname = os.path.realpath(os.path.abspath(pre_commit_vauxoo.get_repo()))
        for v in strcsv2tuple(value):
            try:
            ...
            new_value = os.path.relpath(os.path.realpath(os.path.abspath(new_value)), repo_dirname)
            values += (new_value,)
        return values
